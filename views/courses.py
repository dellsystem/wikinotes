...
def index(request):
	...
	active_courses =list(chain(courses.filter(latest_activity__isnull=False).order_by('-latest_activity__timestamp'),courses.filter(latest_activity__isnull=True)))[:10]
	data = {
		'title': 'Courses',
